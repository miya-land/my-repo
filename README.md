...
- Cache
    - 設定内容
        - キャッシュキー: キャッシュの生成と保存に使用する識別子
        - キャッシュ対象のパス: キャッシュ対象のディレクトリパス、またはファイルパス
        - リストアキー: キャッシュミス時のリストアキーを複数指定する (省略可)
            -   1. keyキーで定義したキャッシュキーと、厳密に一致するキャッシュを探す
            -   2. リストアキーの定義順に、プレフィックスが一致するキャッシュを探す
            - 大半のパッケージマネージャーはキャッシュにないファイルだけをダウンロードするよう振る舞う。部分的にも使えるキャッシュがあれば、それを使ってくれる
                - リストアキーを設定し、以前保存したキャッシュを利用できるようにしておく。(ダウンロード時間が減る)
    - キャッシュキーの設計
        - プラットフォームごとに異なるキャッシュを利用する
            - 最低でもOSごとに分離、CPUアーキテクチャ、言語バージョン、パッケージマネージャーもキャッシュキーの候補
        - 依存関係を更新したときだけキャッシュも変更する
            - package-lock.json のようなロックファイルがある場合、ファイルハッシュをキャッシュキーに指定する
            - 依存関係が更新されたときに、ファイルハッシュがヒットせず、新しくキャッシュを作成する
            ```
            ...
            ```
    - actions/cache
        - キャッシュの保存と復元の両方を担ってくれる
        - actions/cache/save: キャッシュ保存だけを行う
        - actions/cache/restore: キャッシュの復元だけを行う
    - キャッシュの削除
        - 7日以上アクセスされていないキャッシュは自動削除
        - キャッシュの合計サイズは各リポジトリで10GBまで(古いキャッシュから削除)

### 19
- アーティファクト
    - ワークフロー内で生成したファイルのことをアーティファクトという
    - Githubが管理するストレージへ一時的に保存可能
